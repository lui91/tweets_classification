...
def clean_data(df) -> pd.DataFrame:
    ...
    categories = drop_text(categories)
...
def split_categories(df) -> pd.DataFrame:
    ''' split a semicolon separeated string into 
        ...
        '''
    categories_labels = {}
    [categories_labels.update({i:x[0]}) for i, x in enumerate(df["categories"].str.split(pat=";", expand=True).loc[0].str.split(pat="-"))]
    categories = df["categories"].str.split(pat=";", expand=True).rename(columns=categories_labels)
    return categories
